...
class Pais {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $(response).find("forecast > time").each(function() {
                    ...
                    if (day == null){
                        day = { date, tempMin, tempMax, humidity, precipitation, weatherIcon:'' };
                        forecastData.push(day);
                    } else{
                        day.tempMin = Math.min(day.tempMin, tempMin);
                        day.tempMax = Math.max(day.tempMax, tempMax);
                        day.precipitation += parseFloat(precipitation);
                    }
                    if (date === today) {
                ...
                forecastData.slice(0, 5).forEach(day => {
                    ...
                    precipitation.textContent = `Precipitación: ${day.precipitation} mm`;
    ...
    writeDocument(){ 
        document.write("<h3> Coordenadas </h3> <ul> <li> Longitud: " + this.coordenadasMeta.lng + " </li> <li>Latitud: " + this.coordenadasMeta.lat + "</li>")
    }
}
