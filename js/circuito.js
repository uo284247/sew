...
class Circuito {
    ...
    planimetria(files) {
        if (!window.File && !window.FileReader && !window.FileList && !window.Blob) {
            $('section:first').append("<p>¡¡¡ Este navegador NO soporta el API File y este programa puede no funcionar correctamente !!!</p>");
            return;
        }
        
        if (!files || files.length === 0) {
            $('section:first').append("<p>Error: No se seleccionó ningún archivo.</p>");
            return;
        }
        ...
        Array.from(files).forEach(file => {
            if (file.type !== 'application/vnd.google-earth.kml+xml' && !file.name.endsWith('.kml')) {
                $('section:first').append(`<p>Error: El archivo "${file.name}" no es un archivo KML válido.</p>`);
                return;
            }
    
            const divMap = document.createElement("div");
            const main = document.querySelector("section:first-of-type");
            main.appendChild(divMap);
            ...
            reader.onload = function (evento) {
                ...
                // Almacenar los puntos del circuito
                const circuitPoints = [];
        ...
        if (archivo && archivo.type === "text/xml") {
            ...
            lector.onload = function (evento) {
                ...
                let outputHtml = `
                    ...
                    <h3>Fotos</h3>
                    <ul>${fotos.map(foto => `<img src="${foto}"/>`).join('')}</ul>
                    <h3>Videos</h3>
                    <ul>${videos.map(video => `<video controls preload="auto"><source src="${video}"></video>`).join('')}</ul>
